	...
	export default {
		...
		methods: {
			...
			getKey(file) {
				...
				let key = (_self.options.prefix ? _self.options.prefix : 'A') + '.' + _self.createGuid(8, 16) + ext;
				return key;
			},
			propFix() {
				...
				new Uploader(this.url)
					...
					.then((response, file) => {
						...
						if (this.onFileSuccess) {
							let param = Object.keys(this.options).length ? this.options : undefined;
							src = this.onFileSuccess(file, response, param);
						}
						src = src === undefined ? (this.isQiniu ? this.options.domain + response.key : response) : src;
						file.src = src;
			...
			onFileChange() {
				...
				this.onFileCountError && countErrorFiles.length && this.onFileCountError(countErrorFiles);
				this.onFileTypeError && typeErrorFiles.length && this.onFileTypeError(typeErrorFiles);
				this.onFileSizeError && sizeErrorFiles.length && this.onFileSizeError(sizeErrorFiles);
			},
		},
		watch: {
			value() {
